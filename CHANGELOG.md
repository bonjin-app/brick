...
### 추가
- **상품 목록 페이지 나누기** — 전에는 `limit`(기본 24)를 넘는 상품에 **닿을 방법이 없었다**.
  총 개수와 페이저(게시판과 같은 `.brick-pager` 프리미티브)를 낸다. 정렬과 함께 쓰이고,
  없는 쪽을 요청하면 마지막 쪽을 보여준다. 홈의 진열 섹션에는 붙지 않는다(블록 옵션 `paged`)
- **사진 후기** — 후기 작성 폼에서 사진을 3장까지 올린다. API 는 처음부터 `images` 를 받았지만 올릴
  화면이 없었다. 구매자만 올릴 수 있고(비구매자·비로그인은 거부), 코어 이미지 처리를 거쳐 1600px 로
  줄이고 EXIF(촬영 위치)를 지운다. 고르는 즉시 올려 실패 이유가 그 자리에서 드러난다
- **상품 정렬** — 목록 화면에서 손님이 신상품·인기·가격순을 고른다. `<select>` + JS 가 아니라 **링크**라
  주소가 공유되고 뒤로 가기가 자연스럽고 스크립트 없이 동작한다. 홈의 진열 섹션은 운영자가 정한
  순서를 지킨다(블록 옵션 `sortable`). 분류 같은 다른 조건은 유지하고 페이지 번호는 1로 돌아간다
...
- **띠배너**(설정 → 일반) — "5만원 이상 무료배송" 처럼 사이트 전체에 걸리는 공지를 모든 화면 맨 위에
  ...
  다섯 테마가 각자의 색으로 그린다
- **2단 메뉴(드롭다운)** — 코어는 메뉴를 3단까지 저장했지만 테마가 하위를 그리지 않아 쓸 수 없었다.
  다섯 테마가 드롭다운으로 그리고(마우스·키보드 포커스, JS 없이), 좁은 화면에서는 서랍 안에 펼친다.
  관리자 → 메뉴에 **하위 추가**가 생겼고, 쇼핑몰 스타터가 예시 하위 메뉴를 만들어 둔다
...
### 수정
- 드롭다운의 **하위 항목 전부에 "현재 위치"가 붙던 것** — 템플릿 엔진의 `each` 는 항목 스코프에 없는
  키를 부모에서 찾으므로, 상위가 켜져 있으면 자식이 모두 켜졌다(자식마다 `current` 를 명시한다)
