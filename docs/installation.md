...
이미지는 릴리스마다 `ghcr.io/bonjin-app/brick:X.Y.Z` 와 `:latest` 로 발행됩니다.
**linux/amd64 와 linux/arm64** 를 모두 담으므로 Apple Silicon(로컬 Docker Desktop)·AWS Graviton·
...
> 빠뜨려도 아무 오류가 나지 않습니다 — 그래서 Brick 이 대신 알아챕니다. 프록시 헤더가
> 달린 요청이 오는데 신뢰하지 않는 상태라면 관리자 대시보드에 경고가 뜹니다.
> `BRICK_SITE_URL` 도 마찬가지입니다: 기본값(`http://localhost:3000`)인 채로 프로덕션을
> 띄우면 부팅 로그와 대시보드가 경고합니다 — 그대로 두면 비밀번호 재설정·이메일 인증·
> 비회원 주문 조회 **메일 안의 링크가 전부 받는 사람 컴퓨터를 가리킵니다**.
