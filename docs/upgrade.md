...
- **상품 목록의 사진 주소도 함께 잇습니다.** 상품 목록·검색은 `shop_products.thumb_url` 을 쓰는데,
  그 컬럼이 생기기 전에 등록된 상품은 비어 있어 원본을 그대로 내려보냅니다. 이 도구가 미디어에
  이미 있는 썸네일을 찾아 주소만 이어 줍니다(새로 만드는 파일은 없습니다). 외부 URL 을 직접 붙인
  상품은 건너뜁니다 — 그 사진은 우리 것이 아니므로 원본을 계속 씁니다.
