...
**반복 작업은 큐 잡이 스스로 다시 예약하게 하세요.** `setInterval` 을 쓰면
플러그인을 비활성화해도 계속 돌고, 서버가 여러 대면 모두가 돌아 같은 일을
중복합니다. 잡 핸들러 끝에서 `delaySeconds` 로 다음 실행을 예약하면 한
워커에서만 실행되고, 실패해도 큐가 재시도합니다 (ADR-64).

**상태 변화를 감지할 때 각 변경 지점에 붙이지 말고 스윕을 쓰세요.** 재고가
오르는 지점은 반품·취소·관리자 수정·이전 도구 등 여럿이고, **반드시 하나를
빠뜨립니다** — 그리고 빠뜨린 경로의 대기자는 영원히 알림을 못 받는데 아무도
눈치채지 못합니다. "조건이 충족된 것을 주기적으로 찾기"는 경로와 무관하게
동작합니다 (ADR-63).

**`option_id` 처럼 NULL 이 될 수 있는 컬럼을 유니크 인덱스에 넣을 때는
`coalesce` 로 고정값을 주세요.** PostgreSQL 에서 NULL 은 서로 다르게
취급되므로 중복이 막히지 않습니다.
