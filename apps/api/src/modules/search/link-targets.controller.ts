  ...
  async list(@Query("q") q?: string) {
    ...
    groups.push({
      ...
      items: rows.map((r) => ({
        ...
        label: r.title,
        /*
         * 아직 공개되지 않은 페이지도 보여준다. 곧 공개할 페이지를 미리 메뉴에
         * 넣는 것이 실제 순서이고, 숨기면 "왜 목록에 없나"로 헤맨다.
         *
         * 다만 **왜** 공개되지 않았는지는 구분해서 말한다 — 예약된 페이지를
         * "임시 저장" 이라고 하면 운영자는 발행을 잊은 줄 알고 다시 손댄다.
         */
        hint:
          r.status === "published" ? null
          : r.status === "scheduled" ? "예약 발행 (아직 공개 전)"
          : "임시 저장 (아직 공개되지 않음)",
      })),
    });
