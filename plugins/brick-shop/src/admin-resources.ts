...
export const REVIEW_RESOURCE: AdminResource = {
  ...
  searchable: { placeholder: "상품명 또는 작성자" },
  order: 40,
  description:
...
export const INQUIRY_RESOURCE: AdminResource = {
  ...
  searchable: { placeholder: "제목 · 작성자 · 상품명" },
  order: 41,
...
export const COUPON_RESOURCE: AdminResource = {
  ...
  searchable: { placeholder: "쿠폰 코드 또는 이름" },
  order: 30,
  fields: [
...
export const SHOP_SETTINGS_RESOURCE: AdminResource = {
  ...
  fields: [
    ...
    { name: "notifyOrderSms", label: "주문 안내를 문자·알림톡으로도 보낸다", type: "boolean",
      help: "건당 요금이 나갑니다. 문자 발송 확장(예: 알리고)을 설치하고 설정해야 실제로 나갑니다. 알림톡 템플릿을 연결한 안내는 알림톡으로, 나머지는 문자로 나갑니다." },
    ...
    { name: "depositDays", label: "무통장입금 입금 기한(일)", type: "number",
      help: "기한이 지나도록 입금하지 않은 주문은 취소하고 재고를 되돌립니다. 기한은 주문 접수 메일과 주문 조회에 함께 적힙니다. 0 이면 취소하지 않습니다." },
    { name: "unpaidCancelMinutes", label: "온라인 결제 미완료 주문 정리(분)", type: "number",
      help: "카드 등으로 결제를 시작하고 끝내지 않은 주문을 이 시간이 지나면 취소해 재고를 되돌립니다. 결제창을 그냥 닫은 주문이 재고를 붙잡아 다른 손님에게 품절로 보이는 것을 막습니다. 0 이면 취소하지 않습니다." },
