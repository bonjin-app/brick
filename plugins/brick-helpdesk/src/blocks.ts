...
${items}
  ...
  /*
   * 1:1 문의가 **어디에 있는지**를 선언한다.
   *
   * 지금까지 이 블록은 운영자가 페이지를 만들어 올려야만 존재했다 — 회사
   * 홈페이지 스타터만 `/support` 를 만들어 주고, 나머지 사이트에서는 문의
   * 기능을 켜도 손님이 닿을 길이 없었다. 답변 알림도 갈 곳이 없었다.
   *
   * 경로를 스타터와 같은 `support` 로 둔다 — 페이지가 있으면 페이지가 이기므로
   * (렌더러는 페이지를 먼저 찾는다) 회사 사이트의 운영자 페이지는 그대로 쓰이고,
