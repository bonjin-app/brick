...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/E2E-4334%20passing-2ea043.svg" alt="스모크 테스트 4334개" />
...
E2E 스모크 테스트 — 실제 PostgreSQL과 실제 서버 프로세스로 검증합니다 (총 4,334개 항목):
...
| `smoke-portone.sh` | 99 | **포트원 V2 — 결제는 조회로 확인(PAID·금액), 결제 ID 가 이 주문의 것인지, 부분환불에 취소 전 잔액을 실어 이중 환불을 막는다**, 실패·다른 상점 거절 · **가상계좌 — 발급은 입금 대기(계좌·기한을 완료 화면·주문 조회·안내로), 입금은 웹훅으로(내용은 믿지 않고 다시 조회), 결제 미완료 규칙이 입금 전에 지우지 않고 기한이 지나면 취소, 취소 뒤 들어온 입금은 환불** · 창을 닫은 카드 결제도 웹훅이 확정 · 조회가 잠시 실패해도 입금 대기를 잃지 않는다 · **관리자가 결제된 주문을 취소·환불로 바꾸면 돈을 먼저 돌려준다(전이 사전 확인·PG 거절 시 상태 유지)** · 입금된 가상계좌는 손님 계좌로 환불 · 시크릿 비노출 |
...
scripts/          스모크 테스트 44종 + 정적 검사 39종 + OIDC 스텁 + 배포본 생성(build-release.sh)
