...
- **기본 테마 — 라이트·다크 두 벌** — 토큰 한 벌을 더 두면 다크가 따라온다.
...
E2E 스모크 테스트 — 실제 PostgreSQL과 실제 서버 프로세스로 검증합니다 (총 2,721개 항목):

| 수트 | 항목 | 무엇을 못박는가 |
|---|---:|---|
| `smoke-test.sh` | 40 | 설치 · 인증 · 페이지 · 미디어 · 플러그인 로드 |
| `smoke-member.sh` | 111 | 약관 강제 · 동의 이력 · 개인정보 파기 · 주문 보존 · **프로필 이미지·공개 카드·닉네임 변경 주기** |
...
| `smoke-theme.sh` | 99 | 라이트·다크 두 벌 · 토큰 CSS 주입 차단 · 테마 고치면 캐시 갱신 · 랜딩 블록 · **글 상세의 문서 제목** |
| `smoke-moderation.sh` | 30 | 금지 단어(우회 포함)·사칭 이름·금지 도메인·차단 IP(자기잠금)·분류 필수 |
