...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/E2E-3938%20passing-2ea043.svg" alt="스모크 테스트 3938개" />
...
E2E 스모크 테스트 — 실제 PostgreSQL과 실제 서버 프로세스로 검증합니다 (총 3,938개 항목):
...
| `smoke-security.sh` | 89 | **로그인 뒤 돌아갈 주소는 같은 사이트의 경로만(열린 리다이렉트)** · 캡차 · 레이트리밋 · 결제 위조 · 권한 우회 · **CSP** · **캡차 칸 안내** · **비회원 스팸 방어(재입고·문의 캡차·IP 한도)** · **비회원 주문이 메일 발사대가 되지 않는다** | · **설정 실수를 대시보드가 알린다(메일 링크 localhost · 프록시 뒤 IP)** · **가입 스팸(NAT 뒤 다섯 명은 통과 · 같은 이메일 네 번째는 차단)** |
