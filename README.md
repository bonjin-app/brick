...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/E2E-3458%20passing-2ea043.svg" alt="스모크 테스트 3458개" />
...
E2E 스모크 테스트 — 실제 PostgreSQL과 실제 서버 프로세스로 검증합니다 (총 3,458개 항목):
...
| `smoke-storefront.sh` | 179 | 사업자번호 체크섬 · 위시리스트 격리 · 지역비 실수령 · **샘플 상품** · **NEW·BEST 뱃지** · **정렬** · **쪽나눔** · **가격대·품절 필터** · **주문서가 지역 추가 배송비를 미리 보여준다** | · **띄어쓰기 없는 긴 이름** |
...
| `check-reachable-screens.mjs` | 서버에 다 있는데 **닿는 화면이 없는** 기능 (**소셜 연결 해제·포인트 사용·약관 재동의가 그랬다 — 셋 다 API·검증·스모크는 있고 화면만 없었다**) |
...
scripts/          스모크 테스트 37종 + 정적 검사 23종 + OIDC 스텁 + 배포본 생성(build-release.sh)
