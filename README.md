...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/E2E-3545%20passing-2ea043.svg" alt="스모크 테스트 3545개" />
...
E2E 스모크 테스트 — 실제 PostgreSQL과 실제 서버 프로세스로 검증합니다 (총 3,545개 항목):
...
| `smoke-i18n.sh` | 96 | 언어가 실제 렌더를 바꿈 · 즉시 반영 · ko 폴백+로그 · 값은 번역 안 함 · 관리 선언 라벨 · **카드 제목 전수** · **리소스 선언 문자열 전수** · **회원 메뉴·결제수단 이름(손님이 읽는다)** · **금액·날짜 표기와 주문 메일도 언어를 따라간다(손님·관리 화면 모두)** · **코어 경로가 쓰는 플러그인 문구도** · **서버 오류 메시지도 — 플러그인과 코어 양쪽(어느 칸인지는 코드 그대로)** · **페이지 빌더의 블록 서랍(속성 키는 데이터라 그대로)** · 템플릿 잔해 금지 | · **코어·플러그인 메일 전부(재설정·인증·문의 답변·재입고)** |
...
| `check-error-i18n.mjs` | 플러그인이 던지는 **오류 문장에 번역이 없는** 것 (**화면은 전부 영어인데 주문 버튼을 누르면 "재고가 부족합니다." 가 떴다 — 영어 사이트의 손님은 가장 중요한 순간에만 못 읽는 글자를 본다**) |
